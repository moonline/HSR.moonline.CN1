...
\ul
	\li Lösung für Problem kleiner Datenrate: senden von parallelen Objekten mit mehreren Sockets
\ulE

\examp{Datenrate berechnen}{\\
USA RTT = 80ms \\
WindowMax = 2\^16-1 = 65'535 \\
$Rmax = \frac{64000 B}{80ms} = 6.4Mb/s$
}


\exam{\\
\expl{Window Size}{Müsste eigentlich "Recive winow size" heissen. Damit verhindert der Empfänger, dass er überlastet wird. Empfänger legt damit fest, wie viel Platz er im Puffer hat und wie viele Packete der Sender ohne Acknowledge senden darf. Möchte er keine Packete mehr erhalten, sendet er window size 0.}
\expl{Acknowledge \& Sequence Number}{Zuständig für die Zuverlässigkeit der Byteweisen übertragung. Sender nummeriert virtuell jedes Byte. Empfänger sagt mit Acknowledgenumber das höchstwertige erhaltene Byte+1 (Das Byte, das er als nächstes erwartet.}}


\sse{TCP Window Scale}
\ul
	\li Ermöglicht Receive window Size > 64 kB
	\li Scaling option und scaling factor werden beim Verbindungsaufbau ausgehandelt
	\li 
\ulE

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.1.jpg}
	\caption{}
	\label{}
\end{figure}

\sse{duplicate Acknowledge}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.2.jpg}
	\caption{}
	\label{}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.3.jpg}
	\caption{}
	\label{}
\end{figure}

\ul
	\li Empfänger legt Alle Pakete in Puffer
	\li Meldet zurück, dass ihm 21 fehlt
	\li Sobald 21 nachgeliefert wurde, werden Pakete an Applikation weiter gegeben
	\li Pakete grösser 21 werden verwertet
	\li Bei mehreren Lücke keine Chance
\ulE

\sse{Selective Acknowledge - SACK}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.4.jpg}
	\caption{}
	\label{}
\end{figure}
Meldet mit dem Acknowledge, welche er erhalten hat, und welche nicht

\expl{Maximale Anzahl TCP Options}{44 Byte weil Maximalwert von Data Offset $2^4$ = 16 (Max Wert = $2^4$-1 = 15) ist und damit keine längere Länge als 44 Byte möglich sind. Damit ist bei SACK maximal 4 Ranges mit erreichten Paketen möglich, weil SACK noch Overhead hat.}

\sse{Acklowledge Events}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.5.jpg}
	\caption{}
	\label{}
\end{figure}


\sse{Example RTT estimation}
...
Zuerst Slow Start, irgendwann Wechsel zu Linearer Erhöhung
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.8.jpg}
	\caption{}
	\label{}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V12.9.jpg}
	\caption{Beispiel Congestion Avoidance}
	\label{}
\end{figure}

\examp{ISP Datenrate}{Verzögert Pakete, damit Datenrate eingehalten wird}


\ch{Application Layer}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V13.1.jpg}
	\caption{ISO-OSI-Model and Peer-to-Peer
Communication}
	\label{}
\end{figure}
...
\se{NTV - Network Virtual Terminal}
\begin{figure}[H]
	\centering
...
\se{Content Distribution Caching and Proxy Services}
\ul
...
\expl{DNS}{DNS setzt UDP für Transport ein. Um Daten zwischen DNS Servern auszutauschen wird

\expl{TCP/UPD Multicast}{TCP Multicast gibt es nicht, weil zwischen jeder Endstelle eine Verbindung aufgebaut werden müsste.}


\ch{TFTP, FTP} TCP verwendet.}
\se{TFTP - Trivial File Transport Protocol}
\ul
	\li Client wählt ephermeral Port, sendet UDP Paket an Server
	\li TFTP garantiert Übertragungsgarantie auf Applikationsebene.
\ulE

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V14.1.jpg}
	\caption{TFTP 'Connection'}
	\label{}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/V14.2.jpg}
	\caption{TFTP Commands}
	\label{}
\end{figure}

\ul
	\li Nummeriert werden Blöcke, nicht Byte
	\li Block muss immer 512 Byte haben, das Ende wird durch einen Block signalisiert, der nicht 512Byte gross ist. Sind die Daten genau 512 oder ein vielfaches gross, muss noch ein Leerpaket gesendet werden.
	\li ReadRequest (RRQ) gibt an, welches File man gerne hätte und in welchem Mode es übertragen werden soll
	\li WriteRequest WRQ ermöglicht schreiben einer Datei
\ulE
...
\ul
	...
	\li 152.96.123.20,200,15: 1. Nummer*256,+2.*1 \ra 200*256+1*15 = 51215
	\li Server liefert Daten über zweiten Kanal
	\li Probleme bei NAT/PAT: Notwendiges Mapping 152.96.31.12 \ra 192.168.1.11:1173, Zweite Verbindung sollte gehen, weil sie vom Client geöffnet wird.
